...
<head>
    ...
    try {
        $sql = "SELECT * FROM productos p INNER JOIN consejoslavado co ON  co.idconsejoslavado = p.consejoslavado_idconsejoslavado WHERE categorias_id_categoria = 1 order by nombre";
        $query = $conn->prepare($sql);
        $query->execute();
        $results = $query->fetchAll(PDO::FETCH_OBJ);
    } catch (Exception $e) {
        header("location: " . buscarTextoConReturn('WEB', 'paginaa', 'error', '', $_SESSION['idioma']) . "?msg=" . $e->getCode());
    }
    ?>
